...
import BudgetLogo from "../images/BudgetBuddy logo.png";
import SmallLogo from "../images/SmallBudgetBuddy logo.png";
import {
	Drawer,
	DrawerBody,
	DrawerHeader,
...
const Header = () => {
	...
	const { error: deleteError } = useSelector((store) => store.auth);
		...
		<header className="h-[10vh] p-3 px-5 flex justify-between items-center">
			...
			{isLoggedIn ? (
				<p className="hidden sm:block">
					Hi, welcome{" "}
					<strong className="text-green-900">{user?.firstName}</strong>
				</p>
			) : (
				<p className="hidden sm:block text-2xl font-bold text-green-900">
					Welcome My Friend.
				</p>
			)}
